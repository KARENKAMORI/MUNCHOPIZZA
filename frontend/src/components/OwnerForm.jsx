...
const OwnerForm = () => {
    ...
    const createOwnerUsers = (e) => {
        e.preventDefault();
        api.post("/api/owner/", {fname, lname, nationalId, dOB, gender, email, phoneNumber})
        .then((res) => {
            if(res.status === 201) {
                alert("Owner Information Created");
                navigate("/owner");
            } else alert("Failed to create owner information");
            getOwnerUsers();
        })
        .catch((err) => alert(err))
    };
  ...
  return (
    <div className='flex justify-center items-center h-screen'>
        <div className='w-96 p-6 shadow-lg bg-white rounded-md'>
            ...
            <h2 className='text-3xl block text-center font-semibold'>Owner Information</h2>
        <form onSubmit={createOwnerUsers}>
